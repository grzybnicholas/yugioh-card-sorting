...
public:
  ...
  template<typename Comparator>
  int part(Comparator comp, std::string key, int first, int last, int& count);
  
  template<typename Comparator>
  void preQuick(Comparator comp, std::string key, int first, int last, int& count);
