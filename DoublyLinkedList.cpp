...
Node<T>* position1 = getPointerTo(i);
Node<T>* position2 = getPointerTo(j);

if(position1 == nullptr || position2 == nullptr ){
  return;
}

T swap1 = position1->getItem();
T swap2 = position2->getItem();

position1->setItem(swap2);
position2->setItem(swap1);
}
